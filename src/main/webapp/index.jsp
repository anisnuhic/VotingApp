...
<body>
            ...
            <div id="videoContainer" class="video-list">
                ...
                <c:if test="${empty randomVideos}">
                    <div class="empty-state">
                        <h3>Nema dostupnih videa za glasanje.</h3>
    ...
    <script>
        ...
        // NOVA funkcija za čišćenje URL-a nakon što se stranica učita sa dijeljenim videima
        function clearUrlParametersAfterDelay() {
            if (isSharedLink()) {
                // Čekaj 1 sekundu dakorisnik vidi dijeljene videe, zatim ukloni parametre
                setTimeout(function() {
                    if (window.history && window.history.replaceState) {
                        const url = window.location.protocol + "//" + window.location.host + window.location.pathname;
                        window.history.replaceState({ path: url }, '', url);
                        console.log("URL parametri uklonjeni - sljedeći refresh će biti random");
                    }
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        if (response.newVideos && response.newVideos.length === 2) {
                            ...
                            // Ukloni URL parametre nakon glasovanja
                            clearUrlParameters();
                        } else {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Ako je ovo dijeljeni link, prikaži poruku i pripremi za čišćenje URL-a
            if (isSharedLink()) {
                //showMessage('Prikazujem dijeljene videe! Za 2 sekunde ću pripremiti stranicu za random refresh.', 'success');
                clearUrlParametersAfterDelay();
            }
        });
    </script>
</body>
